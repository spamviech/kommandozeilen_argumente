...
impl<'t, T: 't + Clone, E> Argumente<'t, T, E> {
        ...
        Argumente::wert(
            ...
            move |os_str| {
                if let Some(string) = os_str.to_str() {
                    parse(string).map_err(ParseFehler::ParseFehler)
                } else {
    ...
    /// Hilfs-funktion für [`Argumente::wert`].
    #[allow(clippy::too_many_arguments)]
    fn erzeuge_parse_funktion(
        name_lang_präfix: Vergleich<'t>,
        name_lang: NonEmpty<Vergleich<'t>>,
        name_kurz_präfix: Vergleich<'t>,
        name_kurz: Vec<Vergleich<'t>>,
        standard: Option<T>,
        wert_infix_vergleich: Vergleich<'t>,
        meta_var: &'t str,
        parse: impl 't + Fn(OsString) -> Result<T, ParseFehler<E>>,
    ) -> impl 't + Fn(Vec<Option<OsString>>) -> (Ergebnis<'t, T, E>, Vec<Option<OsString>>) {
        move |args| {
            ...
            'args: for arg in args {
                if name_ohne_wert {
                    ...
                    continue;
                } else if let Some(string) = arg.as_ref().and_then(|os_string| os_string.to_str()) {
                    let normalisiert = Normalisiert::neu(string);
                    if let Some(lang) = name_lang_präfix.strip_als_präfix(&normalisiert) {
                        let lang_normalisiert = Normalisiert::neu(lang);
                        let suffixe = filter_prefix(&name_lang, &lang_normalisiert);
                        for suffix in suffixe {
                                ...
                                wert_infix_vergleich.strip_als_präfix(&suffix_normalisiert)
                            {
                                ...
                                // kein match für {lang_name_präfix}{lang_name}[{wert_infix}.*]
                            }
                        }
                        ...
                        if let Some(kurz) = name_kurz_präfix.strip_als_präfix(&normalisiert) {
                            ...
                            if kurz_graphemes
                                .next()
                                .is_some_and(|name| contains_str(&name_kurz, name))
                            {
                                let rest = kurz_graphemes.as_str();
                                let kurz_normalisiert = Normalisiert::neu(rest);
                    ...
                    // Argument kann nicht in einen String konvertiert werden.
                }
    ...
    pub fn wert(
        ...
        Argumente {
            ...
            parse: Box::new(Self::erzeuge_parse_funktion(
                name_lang_präfix,
                name_lang,
                name_kurz_präfix,
                name_kurz,
